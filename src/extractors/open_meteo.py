...
def extract_all_open_meteo(config: dict) -> list[dict]:
    ...
    for location in config["locations"]:
        location_data = extract_open_meteo(
            location["latitude"],
            location["longitude"],
            config["weather"]["daily_variables"],
            config["weather"]["timezone"],
        )
        if location_data is not None:
            data.append({"location": location["name"], "data": location_data})
    return data


def extract_open_meteo_range(
    lat: float, lon: float, start_date: str, end_date: str, daily: list[str], timezone: str
) -> dict | None:
    try:
...
def extract_all_open_meteo_range(config: dict, start_date: str, end_date: str) -> list[dict]:
    data = []
    for location in config["locations"]:
        location_data = extract_open_meteo_range(
            location["latitude"],
            location["longitude"],
            start_date,
            end_date,
